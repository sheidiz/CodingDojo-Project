...
<main class="bg-perfil bg-cover bg-center">
	...
	<div
		...
		<div
			class="mx-auto w-full rounded-xl p-6 shadow-md bg-white dark:bg-neutral-700 dark:text-white">
			<div class="mb-6 text-center">
				<div
					class="mx-auto my-4 size-28 rounded-full bg-slate-100 p-1 drop-shadow-[0_4px_4px_rgba(0,0,0,0.25)] md:p-2 overflow-hidden">
		...
		<div
			class="mx-auto w-full rounded-xl bg-white dark:bg-neutral-700 p-6 shadow-md">
			<!-- Orders Section -->
			<div class="mb-8">
				<h3 class="mb-2 text-xl font-bold dark:text-white">Órdenes
					Pendientes:</h3>
				<c:choose>
						...
						<c:forEach items="${pendingOrders}" var="po">
							<div
								...
								<div class="text-gray-800 dark:text-white">
									<a class="font-semibold" href="/ordenes/${po.id}">¡Solicitaste los servicios de
										${po.postOrder.supplier.fullName}!</a>
									<p class="text-xs">Fecha estimada: <fmt:formatDate value="${po.estimatedDate}" pattern="yyyy-MM-dd" />
									</p>
					...
					<c:otherwise>
						<p class="mb-4 dark:text-white">Por el momento no se hay
				...
				<c:choose>
					<c:when test="${not empty completedOrders}">
						<c:forEach items="${completedOrders}" var="co">
							<div
								...
								<div class="text-gray-800 dark:text-white">
									<a class="font-semibold" href="/ordenes/${co.id}">¡${co.postOrder.supplier.fullName} realizó un trabajo para vos!</a>
									<p class="text-xs">Fecha estimada: <fmt:formatDate value="${co.estimatedDate}" pattern="yyyy-MM-dd" />
									</p>
					...
					<c:otherwise>
						<p class="mb-4 dark:text-white">Por el momento no se
							concretaron ordenes.</p>
					</c:otherwise>
				</c:choose>
			</div>
			...
			<div>
				...
				<c:choose>
					...
					<c:otherwise>
						<p class="mb-4 dark:text-white">Todavía no hay reseñas
							creadas.</p>
					</c:otherwise>
				</c:choose>
			</div>
